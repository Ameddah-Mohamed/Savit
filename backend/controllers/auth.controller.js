...
export const postImg = async (req, res) => {
    try {
        ...
        if (user.profileImg) {
            const publicId = user.profileImg.split("/").slice(-2).join("/").split(".")[0]; // Extract public ID
            await cloudinary.uploader.destroy(publicId);
        }

        // Upload new image to Cloudinary
        const uploadedResponse = await cloudinary.uploader.upload(profileImg, {
            folder: "savit", // Uploads image to the "savit" folder
        });
        ...
        return res.status(200).json({ message: "Profile image updated", profileImg: user.profileImg });

    } catch (error) {
        console.error("ERROR in postImg Controller:", error);
        res.status(500).json({ error: "Internal Server Error" });
    }
};
